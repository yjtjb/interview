...
### 传输实体类的设计？
ZooKeeper是官方推荐的注册中心，稳定性受到肯定，具体的实现在Dubbo源码的dubbo-registry-zookeeper模块中。Zookeeper本身不是为了服务发现而生，它是一个基于ZAB协议实现的可靠的分布式协调系统，它是强一致（CP）、使用基于TCP的私有协议通信。在应对大规模分布式集群上，Zookeeper的实力都是不容小觑的。而Redis注册中心，并没有经过长时间运行的可靠性验证，其稳定性依赖于Redis本身。
